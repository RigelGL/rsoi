...
export class ApiController {
    ...
    async getMyLoyalty(@Headers('X-User-Name') name: string) {
        const loyalty = await this.loyalty.getLoyaltyForUser(name);
        if (!loyalty) throw new InternalServerErrorException();
        return loyalty;
    }
    ...
    async addReservation(@Headers('X-User-Name') name: string, @Body() body: CreateReservationRequest) {
        ...
        if (res.error === 'hotel') throw new NotFoundException(res.error);
        else if (res.error) throw new BadGatewayException(res.error);
